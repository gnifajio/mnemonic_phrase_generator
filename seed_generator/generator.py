...
class Mnemonic:
    ...
    def generate(self):
        ...
        entropy_hash = self._get_hash(entropy)
        indices = self._pick_words(entropy, entropy_hash)
        words = [self.bip39wordlist[indices[i]] for i in range(0, len(indices))]
        word_string = ' '.join(words)
        return word_string

    def get_address(mphrase, derivation_path = '44/60/0/0'):
        addr = BIP32Key.fromEntropy(mphrase)
        for i in dp.split('/'):
            addr = addr.ChildKey(int(i))
        return addr.Address()

    def _pick_words(self, entropy, entropy_hash):
